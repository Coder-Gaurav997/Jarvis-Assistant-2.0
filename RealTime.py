...
GroqAPIKey = env_vars.get("RealTimeAPI")
...
***Find accurate result from the google search results, provided to you***"""
...
def GoogleSearch(query):
    ...
    """
    results = list(search(query, advanced=True, num_results=5))  # Perform the search and get results
    answer = f"The search results for '{query}' are:\n[start]\n"
    for i in results:
        answer += f"Title: {i.title}\nDescription: {i.description}\n\n"  # Format each result
    answer += "[end]"
    return answer  # Return the formatted results

def ChatBot(Query, real_time_info, google_result):
    """
    Generate a response from the chatbot based on the user's query, real-time information, and Google search results.
    
    Parameters:
    Query (str): The user's query.
    real_time_info (str): The current date and time information.
    google_result (str): The results from the Google search.
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "system", "content": google_result},
...
def main(Query):
    ...
    google_result = GoogleSearch(Query)  # Perform a Google search
    Answer = ChatBot(Query, real_time_info, google_result)  # Get the chatbot's response
